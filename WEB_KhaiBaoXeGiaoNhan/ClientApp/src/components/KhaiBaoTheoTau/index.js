...
import { AddVehicleRegisterForShip } from "../../Services/KhaiBaoService/KhaiBaoTheoTau";
const { Search } = Input;
const { Option } = Select;
...
export class KhaiBaoTheoTau extends Component {
  static displayName = KhaiBaoTheoTau.name;
  ...
  onFinish = (values) => {
    ...
    AddVehicleRegisterForShip(values).then((objRespone) => {
      if (objRespone.isSuccess === true) {
        // message.success(objRespone.err.msgString);
  ...
  render() {
    ...
    return (
      <div>
        <HeaderPage
          title="Khai báo danh sách xe chuyển SPL theo tàu"
          description="Khai báo thông tin xe giao/nhận theo tàu"
        />
        <Form
          ref={this.formRef}
            ...
            <Row gutter={24}>
              <Col span={13}>
                <Form.Item
                  ...
                  labelAlign="left"
                  labelCol={{ span: 8 }}
                  label="Mặt hàng (Chỉ chọn 1)"
                  name="productCode"
                  rules={[
                    { required: true, message: "Chưa chọn mặt hàng" },
                  ]}
                >
                  <Select
                    allowClear
